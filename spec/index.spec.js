...
describe("wc: ", () => {
  ...
  describe("checkInputValidity ", () => {
    ...
    it("throws an error if either inputs are invalid", () => {
      expect(() => {
        checkInputValidity("", "some very long string");
      }).toThrow(Error("file not found"));
      expect(() => {
        checkInputValidity(undefined, "some very long string");
      }).toThrow(Error("file not found"));
      expect(() => {
        checkInputValidity("aFileName", null);
      }).toThrow(Error("empty file"));
    });
  });

  describe("when getting the data on individual file: ", () => {
  ...
  describe("when getting data on multiple files, ", () => {
    it("returns the sum of the numbers stored in the array passed to it", () => {
      const result = getTotal([12546, 89767, 8746]);
      expect(result).not.toBeUndefined();
      expect(result).toEqual(111059);
    });
  });
});
